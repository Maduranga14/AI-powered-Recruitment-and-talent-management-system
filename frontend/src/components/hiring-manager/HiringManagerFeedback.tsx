...
export function HiringManagerFeedback({
  ...
  const hasScores = dimensions.every((dimension) => scores[dimension] > 0);
  const canSubmit = Boolean(
    candidate && recommendation && hasScores && evidence.trim().length >= 20
  );
  const submit = () => {
    if (!candidate || !recommendation || !canSubmit) return;
    onSubmitFeedback(candidate.id, recommendation, evidence.trim());
    setSubmitted(true);
  };
  return (
    <motion.div
          ...
          aria-labelledby="scorecard-title">
            ...
            <fieldset
              ...
              className="flex flex-col gap-3 py-4 first:pt-0 sm:flex-row sm:items-center sm:justify-between">
                ...
                <button
                  ...
                  className={`inline-flex h-9 w-9 items-center justify-center rounded-lg border text-sm font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 ${scores[dimension] === score ? 'border-brand-600 bg-brand-600 text-white' : 'border-slate-200 text-slate-500 hover:border-brand-300 hover:bg-brand-50'}`}
            ...
            <fieldset className="mt-7 border-t border-slate-100 pt-6">
              ...
              <div className="mt-3 grid gap-2 sm:grid-cols-3">
                {(
              ['Advance', 'Hold', 'Decline'] as ManagerRecommendation[]).
              map((option) =>
              <button
                key={option}
                type="button"
                onClick={() => setRecommendation(option)}
                className={`rounded-xl border px-3 py-3 text-sm font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 ${recommendation === option ? option === 'Advance' ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : option === 'Hold' ? 'border-amber-500 bg-amber-50 text-amber-700' : 'border-red-500 bg-red-50 text-red-700' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
            ...
            <div className="mt-7">
              ...
              placeholder="What did you observe? Mention examples, tradeoffs, or moments that shaped your recommendation…"
              hint="At least 20 characters required to submit." />
            
              <p className="mt-2 text-right text-xs text-slate-400">
                {evidence.trim().length}/20 minimum
              </p>
            </div>
            <div className="mt-7 flex flex-col gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:items-center sm:justify-between">
              <p className="flex items-center gap-1.5 text-xs text-slate-500">
                <StarIcon className="h-4 w-4 text-amber-500" /> All ratings and
