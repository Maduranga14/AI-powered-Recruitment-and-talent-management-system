    ...
    >
        ...
        <div className="grid gap-3 md:grid-cols-[1fr_210px]">
          ...
          <label className="relative">
            ...
            >
              ...
              <option className="bg-slate-900 text-white">Healthy</option>
              <option className="bg-slate-900 text-white">Review</option>
              <option className="bg-slate-900 text-white">Restricted</option>
            </select>
      ...
      <div className="mt-5 flex items-center justify-between text-white">
        <p className="text-sm text-slate-400">
          <strong className="text-white">{visible.length}</strong> organizations shown
        </p>
        <button
      ...
      {visible.length ? (
        ...
        >
          <div className="hidden grid-cols-[minmax(220px,1.4fr)_100px_100px_100px_110px_100px_120px] gap-4 border-b border-slate-800 bg-slate-950/60 px-5 py-3 text-[11px] font-bold uppercase tracking-wider text-slate-400 lg:grid">
          ...
          <div className="divide-y divide-slate-800">
            {visible.map((organization) => (
              <div
                ...
                className="grid w-full grid-cols-1 items-center gap-3 px-5 py-4 text-left hover:bg-slate-800/60 cursor-pointer transition-colors lg:grid-cols-[minmax(220px,1.4fr)_100px_100px_100px_110px_100px_120px] lg:gap-4"
              >
                <div className="flex items-center gap-3">
                  <span className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-brand-500/20 text-teal-300 border border-brand-500/30 font-display text-sm font-extrabold">
                    {organization.initials}
                  </span>
                  <div className="min-w-0">
                    <p className="font-bold text-white text-sm truncate">{organization.name}</p>
                    <p className="text-xs text-slate-400 truncate">{organization.sub}</p>
                  </div>
                </div>
                <div className="text-xs font-semibold text-slate-300">
                  <Badge tone="brand" className="bg-brand-500/20 text-teal-300 border border-brand-500/30">{organization.plan}</Badge>
                </div>
                ...
                <div className="text-xs text-slate-400">{organization.monthlyUsage}</div>
                <div className="flex items-center justify-start lg:justify-end gap-2 pt-2 lg:pt-0 border-t lg:border-t-0 border-slate-800">
                  <Button
                    size="sm"
                    onClick={(e) => {
                      e.stopPropagation();
                    ...
                    className="bg-brand-600 hover:bg-brand-500 text-white text-xs font-bold px-3 py-1.5 flex items-center gap-1.5 shadow-md"
                  >
                    <Building2Icon className="h-3.5 w-3.5" /> Manage Workspace
                  </Button>
                </div>
              </div>
            ))}
              ...
              <div className="flex items-center justify-between border-b border-slate-800 pb-4">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-300">Verify company tax details & setup tenant</p>
                  </div>
                </div>
                <button
                  onClick={() => setIsModalOpen(false)}
              ...
              <form onSubmit={handleCreate} className="mt-4 space-y-4 max-h-[70vh] overflow-y-auto pr-1">
                ...
                <div>
                  ...
                  <Input
                    ...
                    icon={<ShieldCheckIcon className="h-4 w-4 text-teal-400" />}
                    required
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Input
                      ...
                      icon={<GlobeIcon className="h-4 w-4 text-teal-400" />}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full rounded-xl border border-slate-700 bg-slate-800 px-3.5 py-2 text-sm font-semibold text-white outline-none focus:border-teal-400 focus:ring-2 focus:ring-teal-400/20"
                    >
                      <option value="Starter" className="bg-slate-900 text-white">Starter</option>
                      <option value="Growth" className="bg-slate-900 text-white">Growth</option>
                      <option value="Scale" className="bg-slate-900 text-white">Scale</option>
                    </select>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full rounded-xl border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-white outline-none focus:border-teal-400 focus:ring-2 focus:ring-teal-400/20 placeholder:text-slate-400"
                  />
                </div>
